...
def getSurvivalTime_dp(A, B, X, Y, Z, memo, loc):
    if A <=0 or B <=0:
        return 0
    ...
    for key in memo.keys():
        if curr.a ==  key.a and curr.b == key.b:
            return memo[curr]

    if loc == 0:
        temp = 1 + max(getMaxSurvivalTime_dp(A + Y.a, B + Y.b, X, Y, Z, memo, 1 ),
                       getMaxSurvivalTime_dp(A + Z.a, B + Z.b, X, Y, Z, memo, 2))

    if loc == 1:
        temp = 1 + max(getMaxSurvivalTime_dp(A + X.a, B + X.b, X, Y, Z, memo, 0 ),
                       getMaxSurvivalTime_dp(A + Z.a, B + Z.b, X, Y, Z, memo, 2))

    if loc == 2:
        temp = 1 + max(getMaxSurvivalTime_dp(A + X.a, B + X.b, X, Y, Z, memo, 0 ),
                       getMaxSurvivalTime_dp(A + Y.a, B + Y.b, X, Y, Z, memo, 1))
...
def getMaxSurvivalTime_recurcive(X, Y, Z, A, B, isRecurcive = True):
    ...
    if isRecurcive:
        ...
        a = getSurvivalTime_dp(A + X.a, B + X.b, X, Y, Z, memo, 0)
        b = getSurvivalTime_dp(A + Y.a, B + Y.b, X, Y, Z, memo, 1)
        c = getSurvivalTime_dp(A + Z.a, B + Z.b, X, Y, Z, memo, 2)
